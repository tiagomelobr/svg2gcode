use std::fs;
use svg2gcode_wasm::{convert_svg, GCodeConversionOptions};
...
fn test_conversion(svg_content: &str, tolerance: f64, min_arc_radius: Option<f64>, description: &str) {
    let config = GCodeConversionOptions {
        conversion: svg2gcode_wasm::ConversionConfig {
            tolerance,
            feedrate: 3000.0,
            dpi: 96.0,
            origin_x: None,
        ...
        machine: svg2gcode_wasm::MachineConfig {
            ...
            tool_on_sequence: Some("G4 P0.05\nG1 Z1\nG4 P0.05".to_string()),
            tool_off_sequence: Some("G4 P0.05\nG1 Z0\nG4 P0.2".to_string()),
            begin_sequence: Some("; Document Start\nG21\nG17\nG90\nF10000\nG0 Z0\nG4 P0.2\nG0 X0 Y0".to_string()),
            end_sequence: Some("; END\nG0 Z0\nG4 P0.2\nG0 X0 Y0 F10000".to_string()),
            between_layers_sequence: Some("M0".to_string()),
        },
        postprocess: svg2gcode_wasm::PostprocessConfig {
            checksums: false,
            line_numbers: false,
    ...
    // Convert to JavaScript value for the WASM function
    let config_value = serde_wasm_bindgen::to_value(&config).unwrap();
    
    match convert_svg(svg_content, &config_value) {
        Ok(gcode) => {
